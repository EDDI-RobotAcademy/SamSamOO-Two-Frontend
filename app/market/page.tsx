...
export default function MarketPage() {
    ...
    const fetchProducts = async () => {
        setLoading(true);
        const res = await fetch(`http://localhost:33333/market-data/search?query=${query}`);
...
const fetchReviews = async (catalogId: string) => {
    ...
    const res = await fetch(
        `http://localhost:33333/market-data/reviews?catalogId=${catalogId}`
    );
            ...
            <div className="grid grid-cols-3 gap-4">
                {products.map((p: any) => (
                    <div key={p.productId} className="border p-3 rounded shadow">
                        ...
                        <h2 className="font-bold mt-2" dangerouslySetInnerHTML={{ __html: p.name }} />
