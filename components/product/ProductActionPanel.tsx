...
export default function ProductActionPanel({
  ...
  // ? 부모에서 주면 우선, 없으면 기본(ANALYZED)로 동작
  const allowPdf =
    typeof canDownloadPdf === 'boolean'
      ? canDownloadPdf
      ...
      <div className="space-y-3">
        ...
        {onRecollect && (
          <>
            <button
              ...
              onClick={() => onRecollect()}
              disabled={!canRecollect || !!pendingFlags.recollect || busy}
        ...
        {product.analysis_status === 'PENDING' && (
          <button
            ...
            disabled={busy || pendingFlags.crawl}
        ...
        {product.analysis_status === 'COLLECTED' && (
          <button
            ...
            disabled={busy || pendingFlags.analyze}
